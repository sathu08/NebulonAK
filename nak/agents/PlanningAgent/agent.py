"""
nak.agents.PlanningAgent.agent -- generated skeleton (confirm-gated).
...
Usage:
    from nak.agents.PlanningAgent.agent import Agent
...
def _parse_step_answer(raw):
    ...
    try:
        ...
        # fallback for markdown-wrapped JSON: first { to last }
...
class Agent:
    """PlanningAgent — handles: Reusable planning agent: creates implementation plans, roadmaps and designs for any task or stack (not tied to one framework)"""
    ...
    def __init__(self, brain: Optional[Brain] = None, system_prompt: Optional[str] = None) -> None:
        ...
        # (no "system"). Keep role_hint as plain context, never sent as a message.
        default_hint = "You are PlanningAgent, a helpful assistant for reusable planning."
    ...
    def run_sync(self, text: str, messages: Optional[List[Dict[str, str]]] = None, **kwargs: Any) -> Dict[str, Any]:
        ...
        for _ in range(max_turns):
            ...
            if approve_tool_use(tname, targs):
                try:
                    ...
                    # executors return {"error": ...} JSON instead of raising,
                    # so success must be read from the result, not the absence
                    # of an exception.
                    try:
        ...
        # User reference: every completed run saves its output to nak_plan.md
        # (repo root). Best-effort: error markers are never saved as plans,
        # and a failed save never fails the run.
        if answer and not answer.startswith(("[brain error", "(no answer")):
            try:
                execute_plugin(self.brain, "write_file",
                               {"path": "nak_plan.md", "content": answer, "mode": "overwrite"})
            except Exception:
                pass
        return {"answer": answer, "turns": turns,
